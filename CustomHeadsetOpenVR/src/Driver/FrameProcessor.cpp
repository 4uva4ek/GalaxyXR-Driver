...
bool FrameProcessor::EnsureShaders(){
	...
	if(checkFile){
		...
		// 2026-09-19 SDR10 baseline: pin the EXACT source in the log
		// (FNV-1a 64). the embedded fallback declares the same cbuffer
		// (layout identity) but does not apply the color controls, so
		// the hash is what separates the two in provenance
		uint64_t srcHash = 14695981039346656037ULL;
		for(size_t i = 0; i < source.size(); i++){
			srcHash ^= (unsigned char)source[i];
			srcHash *= 1099511628211ULL;
		}
		DriverLog("FrameProcessor: pixel shader ready (%s, srcHash=0x%016llx, gaze ring support: %s, calib dot support: %s, warped overlays: %s, tuner ring: %s, world grid: %s, align shift: %s, band segments: %s, fxaa: %s, fxaaPass: %s)",
			fileTime ? "from file" : "embedded", (unsigned long long)srcHash, hasGazeRing ? "yes" : "NO - stale hlsl?",
...
bool FrameProcessor::ProcessEye(ID3D11Texture2D* texture, const vr::VRTextureBounds_t &bounds, int eye, int slice, const FrameProcessSettings &settings){
	...
	// constants. 2026-09-19 SDR10 baseline: the color chain reads the
	// EFFECTIVE policy (neutral while active, exact passthrough of config
	// while inactive); cas / fxaa / distortion / safety read config as
	// before because they are not baseline-owned
	...
	double contrastMult = policy.contrast / 50.0;
	double contrastMid = policy.contrastMidpoint / 100.0;
	constants.contrastMult = (float)contrastMult;
	constants.contrastOffset = (float)(-contrastMid * contrastMult + contrastMid);
	constants.contrastLinear = policy.contrastLinear ? 1.0f : 0.0f;
	constants.outGamma = (float)policy.gamma;
	...
	constants.ditherEnable = policy.dither ? 1.0f : 0.0f;
	// black-floor FIX state is effective (baseline disables it); the ramp
	// bar is the user's debug overlay and stays a stored config value
	constants.bfRampBar = config.blackFloor.rampBar ? 1.0f : 0.0f;
	constants.bfRangeMode = (float)policy.blackFloorRangeMode;
	constants.bfShadowFloor = policy.blackFloorShadowLift ? (float)policy.blackFloorFloorCode : -1.0f;
	constants.bfKnee = (float)policy.blackFloorKneeCode;
	constants.bfBlackPoint = (float)policy.blackFloorBlackPointCode;
	...
	}
	{
		...
		// (effective value: the baseline forces it to 1)
		double b = settings.policy.brightness;
		if(b < 0.0){ b = 0.0; }
		if(b > 4.0){ b = 4.0; }
	...
	{
		// effective brightness (2026-09-19 SDR10 baseline: neutral 1 while
		// active, exact stored value while inactive)
		double b = settings.policy.brightness;
		if(b < 0.0){ b = 0.0; }
		if(b > 4.0){ b = 4.0; }
	...
	// effective tint/matrix (2026-09-19 SDR10 baseline: neutral 1/1/1 and
	// no matrix while active, exact stored values while inactive)
	constants.colorMultiplier[0] = (float)policy.colorMultiplier.r;
	constants.colorMultiplier[1] = (float)policy.colorMultiplier.g;
	constants.colorMultiplier[2] = (float)policy.colorMultiplier.b;
	constants.colorMultiplier[3] = 1.0f;
	if(policy.srgbMatrix.size() == 9){
		constants.matrixEnable = 1.0f;
		for(int i = 0; i < 3; i++){
			constants.matR[i] = (float)policy.srgbMatrix[i];
			constants.matG[i] = (float)policy.srgbMatrix[3 + i];
			constants.matB[i] = (float)policy.srgbMatrix[6 + i];
		}
	}else{
