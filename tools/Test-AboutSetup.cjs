...
(async()=>{
  ...
  await test('No installation means no native runtime check or launch',async()=>scenario(async(h,c)=>{let calls=0;h.api.invoke=async()=>{calls++;return runtime()};await c.startup.refresh();assert.equal(await c.startup.start(),false);assert.equal(calls,0);},h=>h.fixture.files.delete(h.fixture.runtime+'/drivers/GalaxyXRNative/driver.vrdrivermanifest')));
