...
(async()=>{
 ...
 test('All four settings pages use the shared grouping helper',()=>{for(const file of ['driver-settings-page','stream-frame-page','distortion-profile-page','app-settings-page'])assert.match(fs.readFileSync(root+'/src-lit/features/'+file+'.ts','utf8'),/sectionCards\((body|parts)\)/);});
