...
import { get_executable_path, restart_vrcompositor, run_process_sync, register_galaxyxr_driver,
  ...
  clean_legacy_galaxyxr_identity, IdentityCleanupReport } from '../tauri_wrapper';
...
import { cleanJsonComments, suspendFileWrites, resumeFileWrites } from '../helpers';
import { steamVRSettingsDiff } from './steamvr-settings-diff';
...
export class SystemDiagnosticService implements OnDestroy {
  ...
  async watchSteamVRSettings() {
    ...
    if (!directory || directory === this.watchedSteamVRConfig) return;
    this.unwatchSteamVRConfig?.();
    const subject = new Subject<void>();
    const subscription = subject.pipe(debounceTime(50)).subscribe(async () => {
      try { this._steamVrConfig.set(await this.getSteamVRSettings()); }
      catch (error) { console.warn('Cannot read SteamVR settings', error); }
    });
    try {
      ...
      subject.complete();
      console.warn('Cannot watch SteamVR settings', error);
    }
    try { this._steamVrConfig.set(await this.getSteamVRSettings()); }
    catch (error) { console.warn('Cannot read SteamVR settings', error); }
  }
  ...
  private async inspectDriverPackage(path: string, name = customHeadsetDriverName): Promise<string | undefined> {
    ...
    try {
      const manifest = JSON.parse(cleanJsonComments(await readTextFile(await join(path, 'driver.vrdrivermanifest'))));
      if (manifest.name !== name || !await exists(await join(path, 'bin', 'win64', `driver_${name}.dll`))) return undefined;
      return typeof manifest.version === 'string' ? manifest.version : '0.0.0';
    } catch { return undefined; }
  }
  public async checkDriverInstalled(force = false) {
    if (this.installing && !force) return !!this._driverInstalled();
    const task = this.inspectInstalledDriver(this.installationGeneration);
  ...
  private async inspectInstalledDriver(generation: number) {
    ...
    if (steamVrPath) {
      ...
      if (version) {
        ...
        if (!wasInstalled || !this.dss.values()) await this.dss.ensureEditableSettings();
        if (!current()) return false;
        await this.watchSteamVRSettings();
        if (!current()) return false;
  ...
  async uninstallDriver() {
    ...
    try {
      ...
      if (!this.driverDataRemoved) resumeFileWrites();
      await this.dialog.message($localize`Uninstall Failed`, `${e}`);
      return false;
    } finally {
      this.installing = false;
      this._installingDriver.set(false);
  ...
  public async getSteamVRSettings() {
    const path = await this.getSteamVRConfigFilePath();
    if (path) {
      return await exists(path) ? JSON.parse(cleanJsonComments(await readTextFile(path))) : {};
    }
  }
