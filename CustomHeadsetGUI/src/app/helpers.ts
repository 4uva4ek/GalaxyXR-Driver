...
let fileWritesSuspended = false;
let fileWriteGeneration = 0;
const fileWriterPaths = new Set<string | Promise<string>>();
export function resumeFileWrites() { fileWritesSuspended = false; }
export async function suspendFileWrites() {
  fileWritesSuspended = true;
  fileWriteGeneration++;
  // Drain in-flight writers. Queued/throttled writers check the suspension
  // inside the same lock, so they cannot recreate deleted driver config.
  await Promise.all([...fileWriterPaths].map(async path => {
...
export function debouncedFileWriter(path: string | Promise<string>, tempFileDir: string | Promise<string>, directWrite?: (() => boolean)): DebouncedFileWriter {
  fileWriterPaths.add(path);
  const saveSbj = new Subject<{ content: string; generation: number }>();
  let savingFile = false
  saveSbj.pipe(throttleTime(50, undefined, { trailing: true })).subscribe(async ({ content, generation }) => {
    await navigator.locks.request(`saving file_${await path}`, async () => {
      if (fileWritesSuspended || generation !== fileWriteGeneration) return;
      savingFile = true;
      try {
        if (directWrite?.()) {
          try {
  ...
  return {
    save: (content: string) => {
      if (!fileWritesSuspended) saveSbj.next({ content, generation: fileWriteGeneration })
    },
