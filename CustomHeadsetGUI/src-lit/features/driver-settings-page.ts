...
export class DriverSettingsPage extends BasePage {
  ...
  render() {
    ...
    if (this.section('headset')) {
      ...
      body.push(
        ...
        fieldRow(t('Native Render Resolution'), html`<app-switch .checked=${!!gx.nativeResolution} @change=${(e: CustomEvent) => { gx.nativeResolution = e.detail; save(); }}></app-switch>`, {
          tip: "Ask SteamVR to render at the Galaxy XR's native per-eye size. This improves the requested resolution but can increase GPU load. Restart SteamVR to apply it.\n\nRequests 3552 × 3840 per eye at 90 Hz. Headset-specific renderWidth, renderHeight, overrideRenderWidth, overrideRenderHeight, and displayFrequency are written to vrlink_xrvst2ue when vrlink Headset Profile is On, otherwise to the previous driver_vrlink section. SteamVR’s global preferredRefreshRate remains in steamvr because SteamVR reads it there. Turning this off restores only unchanged values backed by this driver’s recovery journal; settings owned by another tool are not removed.",
        }),
        ...(advanced ? [fieldRow(t('vrlink Headset Profile'), html`<app-switch .checked=${!!gx.vrlinkHeadsetProfile} @change=${(e: CustomEvent) => { void this.changeIdentitySetting('vrlinkHeadsetProfile', e.detail, e.currentTarget as HTMLElement & { checked: boolean }); }}></app-switch>`, {
          tip: "Keep this on to use the dedicated Galaxy XR settings profile. Turning it off can leave SteamVR using Unknown or the identity from patched Steam Link instead. It also switches settings back to their previous destinations; restart SteamVR afterwards.\n\nEnabled by default. On selects the exact steamvr.vrsettings section vrlink_xrvst2ue, regardless of the model name reported by a patched APK. Baseline/capability requests, stream size and bandwidth, render overrides, diagnostics, and supported extra vrlink keys use that destination. Off preserves the previous split: tuning in driver_vrlink, baseline/capabilities in vrlink_<original model> (xrvst2ue if no model is available). An active SDR 10-bit baseline still requests capabilities in that legacy destination; it does not silently turn this saved switch back on. Driver enable/block keys and SteamVR global settings keep their required sections. A restart/reconnect is needed; whether a Steam Link version consumes every profile key must be checked in its logs.",
        })] : []),
      );
    ...
    return html`<app-system-ready .ctx=${this.ctx}>${body}</app-system-ready>`;
  }
}
