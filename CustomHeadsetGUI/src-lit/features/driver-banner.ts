...
import { customHeadsetDriverName, vendor } from '../environment';
...
export class DriverEnableBanner extends BasePage {
  ...
  private async enableDriver(): Promise<void> {
    await this.runAction(() => vendor
      ...
      : this.ctx.sds.enableSteamVRDriver(customHeadsetDriverName));
  }
  ...
  render() {
    ...
    const blocked = sds.isDriverBlocked(settings, customHeadsetDriverName);
    if (sds.getSteamVRDriverEnableState(settings, customHeadsetDriverName) && !neutral) return html``;
