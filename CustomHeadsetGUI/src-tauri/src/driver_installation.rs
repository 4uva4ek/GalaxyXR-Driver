...
fn load_journal(ctx: &Context) -> Result<Value> {
    ...
    let journal = read_json(&ctx.journal())?;
...
fn validate_package_path(path: &Path, current_exe: &Path) -> Result<PathBuf> {
    ...
    if [".git", "src", "CustomHeadsetGUI", "CustomHeadsetOpenVR", "Cargo.toml", "CMakeLists.txt"].iter().any(|p| path.join(p).exists()) {
...
mod tests {
    ...
    fn install_uninstall_reinstall_preserves_bundle_and_restores_settings() {
        let f=Fixture::new();let source=f.package("bundle/GalaxyXRNative");let exe=f.root.join("bundle/CustomHeadsetGUI/gui.exe");
