...
| Profile On | Before connection, tuning settings reach `driver_vrlink` and copies remain in `vrlink_xrvst2ue`, `vrlink_Oculus Quest Pro` and `vrlink_PICO 4 Pro`. Capability requests remain per-model; known Quest/PICO models use built-in capabilities. After restart/reconnect verify actual render size, stream width and bandwidth mode, not merely saved copies. Originals remain independently journaled | NOT RUN |
