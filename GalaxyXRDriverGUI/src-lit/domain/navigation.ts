...
export function visibleRoutes(available: boolean): readonly Route[] {
  return available ? ROUTES : ['about', 'app-settings'];
}

export function parseRoute(hash: string): Route {
  const value = hash.replace(/^#\/?/, '').replace(/^\/+/, '');
  return (ROUTES as readonly string[]).includes(value) ? value as Route : 'about';
}

export function permittedRoute(requested: Route, available: boolean): Route {
  return available || requested === 'app-settings' || requested === 'about' ? requested : 'about';
}
