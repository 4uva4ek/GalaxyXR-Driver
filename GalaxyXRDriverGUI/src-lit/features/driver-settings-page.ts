    ...
    if (this.section('headset')) {
      ...
      body.push(
        ...
        fieldRow(t('Native Render Resolution'), html`<app-switch .checked=${!!gx.nativeResolution} @change=${(e: CustomEvent) => { gx.nativeResolution = e.detail; save(); }}></app-switch>`, {
          tip: t("Ask SteamVR to render at the Galaxy XR's native per-eye size. This improves the requested resolution but can increase GPU load. Restart SteamVR to apply it.\n\nRequests 3552 × 3840 per eye at 90 Hz. renderWidth, renderHeight, overrideRenderWidth, overrideRenderHeight, and displayFrequency are written to driver_vrlink, where VRLink reads the tuning settings. With vrlink Headset Profile On, the same values are also copied to vrlink_xrvst2ue, vrlink_Oculus Quest Pro and vrlink_PICO 4 Pro. SteamVR's global preferredRefreshRate remains in steamvr. Each section's original values are journaled independently. Turning this off restores only unchanged journal-owned values; external edits are preserved."),
        }),
