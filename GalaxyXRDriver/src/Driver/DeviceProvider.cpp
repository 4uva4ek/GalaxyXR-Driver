...
void GalaxyXRDeviceProvider::AnchorReleaseGesture(uint32_t openVRID){
	double now = std::chrono::duration_cast<std::chrono::microseconds>(
		std::chrono::steady_clock::now().time_since_epoch()).count() / 1000000.0;
	std::lock_guard<std::mutex> guard(poseLogLock);
...
void GalaxyXRDeviceProvider::ComputeRingSecant(const VelFixState &state, bool useSmoothed, double secantVel[3], double secantAng[3]){
...
bool GalaxyXRDeviceProvider::DeriveMotion(uint32_t openVRID, const vr::DriverPose_t &pose, double derivedVel[3], double derivedAng[3], double secantVel[3], double secantAng[3]){
	double now = std::chrono::duration_cast<std::chrono::microseconds>(
		std::chrono::steady_clock::now().time_since_epoch()).count() / 1000000.0;
	std::lock_guard<std::mutex> guard(poseLogLock);
	VelFixState &state = velFixStates[openVRID];
...
void GalaxyXRDeviceProvider::SetAlignerGrip(bool active, const double gripCm[2][3]){
	{
		std::lock_guard<std::mutex> guard(poseLogLock);
